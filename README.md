...
sequenceDiagram
    ...
    Investigator->>UI: Upload cranial image
    Investigator->>UI: Configure parameters

    UI->>CV: Send skull image
    ...
    UI->>AI: Send contour map + prompt
    AI->>GPU: FP16 inference
    GPU->>AI: Diffusion sampling

    AI-->>UI: Return candidate hypotheses
    UI-->>Investigator: Display facial hypothesis gallery
```
...
ForensiFace relies on pretrained generative and conditioning models rather than a project-specific diffusion model trained from scratch.
...
### Standalone Candidate Generation
